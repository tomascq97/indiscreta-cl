...
function assertSafeEnvironment() {
  ...
  if (
    ...
    throw new MedusaError(
      ...
      "Shipit test product setup requires a local database and disabled shipment creation",
    )
  }
}

export default async function setupShipitTestProduct({
  container,
}: ExecArgs) {
  ...
  const [
    ...
    query.graph({ entity: "stock_location", fields: ["id", "sales_channels.id"] }),
    query.graph({ entity: "sales_channel", fields: ["id"] }),
  ])
